...
public class JdbcEnrollmentDao implements EnrollmentDao {
    ...
    public synchronized Enrollment save(Enrollment enrollment) {
        String sql = "INSERT INTO enrollments (student_id, course_id, completed) VALUES (?,?,?)";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setLong(1, enrollment.getStudentId());
            ps.setLong(2, enrollment.getCourseId());
            ps.setBoolean(3, enrollment.isCompleted());
            ps.executeUpdate();
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) enrollment.setId(rs.getLong(1));
            }
            return enrollment;
        } catch (SQLException ex) {
            // if unique constraint violation, throw a clear message
            if (ex.getSQLState() != null && ex.getMessage().toLowerCase().contains("duplicate")) {
    ...
    public Enrollment update(Enrollment enrollment) {
        String sql = "UPDATE enrollments SET student_id=?, course_id=?, completed=? WHERE id=?";
        try (Connection conn = DBConnection.getConnection();
    ...
    public Optional<Enrollment> findById(Long id) {
        String sql = "SELECT id, student_id, course_id, completed FROM enrollments WHERE id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setLong(1, id);
    ...
    public List<Enrollment> findAll() {
        String sql = "SELECT id, student_id, course_id, completed FROM enrollments";
    ...
    public Optional<Enrollment> findByStudentAndCourse(Long studentId, Long courseId) {
        String sql = "SELECT id, student_id, course_id, completed FROM enrollments WHERE student_id=? AND course_id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<Enrollment> findByStudentId(Long studentId) {
        String sql = "SELECT id, student_id, course_id, completed FROM enrollments WHERE student_id=?";
        List<Enrollment> list = new ArrayList<>();
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setLong(1, studentId);
